...
def main():
  ...
  for trafoStr in trafoStrs:
    ...
    X = X[K,:]
    ax.plot(*X.T, "k.", mec=backgroundColor, mew=0.3, ms=4,
  ...
  fig = Figure.create(figsize=(3, 2), scale=0.7)
  ax = fig.gca()
  colorbar = fig.colorbar(
      contour, orientation="horizontal", fraction=1,
      aspect=15, ticks=np.arange(-6, -3+0.01, 1), format="$10^{%u}$")
