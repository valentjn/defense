...
def plotCuboid(ax, trafo, corner, size, faceColor,
               edgeColor=None, zorder=None):
...
def main():
  ...
  for s in range(3):
    if s < 2:
      ...
      kwargs = (lambda r: {"color" : "r"})
      boxColor = "r"
      contourColors = None
    
    for q in range(4):
      scale = (0.42 if s == 0 else 0.27)
      fig = Figure.create(figsize=(5, 3), scale=scale)
      ax = fig.gca()
      ...
      ax.plot(x, y, "-", clip_on=False, color=boxColor)
  ...
  for s in range(3):
    ...
    for q in range(2):
      scale = (0.63*[0.62, 0.8][q] if s == 0 else 0.35*[0.62, 0.73][q])
      fig = Figure.create(figsize=(4, 4), scale=scale)
      ...
      plotCuboid(ax, trafo, [(1+c)/2, (1-a)/2, (1-a)/2], [(1-c)/2, a, a],
                 faceColor, edgeColor=edgeColor, zorder=0)
      plotCuboid(ax, trafo, [(1-b)/2, (1+c)/2, (1-b)/2], [b, (1-c)/2, b],
                 faceColor, edgeColor=edgeColor, zorder=1)
      plotCuboid(ax, trafo, [(1-c)/2, (1-c)/2, 0],       [c, c,       1],
                 faceColor, edgeColor=edgeColor, zorder=2)
      plotCuboid(ax, trafo, [0,       (1-a)/2, (1-a)/2], [(1-c)/2, a, a],
                 faceColor, edgeColor=edgeColor, zorder=3)
      plotCuboid(ax, trafo, [(1-b)/2, 0,       (1-b)/2], [b, (1-c)/2, b],
                 faceColor, edgeColor=edgeColor, zorder=4)
      ...
      if (q == 1) and (s == 0):
        ...
        ax.plot(xx, yy, "--", zs=zz, clip_on=False, color=boxColor)
        t = np.pi/2 - theta1/2
        ax.text(
            center[0]+0.8*radius*np.cos(t), 1, center[2]+0.8*radius*np.sin(t),
            r"\contour{white}{$\theta_1$}",
