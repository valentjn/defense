...
def main():
  ...
  for q in range(2):
    fig = Figure.create(figsize=((2.35, 2.3) if q == 0 else (3.05, 2.3)))
    ...
    if q == 0:
      ...
      for model in models[q]:
        ...
        ax1.plot(Ns, complianceApprs, "^--", color="C2", ms=3)
        ax1.plot(Ns, complianceReals, "v-", color="C2", ms=3)
      ...
      ax2.set_ylabel(r"$\abs{\compliance[\opt,\ast] -"
                     r"\complianceintp[\opt,\ast]}$")
    else:
      ax2 = fig.gca()
    
    for j, model in enumerate(models[q]):
      Ns, complianceApprs, complianceReals = selectData(data, model, NMax)
      color = "C{}".format(j+q+3)
  ...
  helper.plot.addCustomLegend(ax, [
      ...
      {
        "label"  : r"\rlap{$\abs{\compliance[\opt,\ast] -"
                   r"\complianceintp[\opt,\ast]}$}",
        "ls"     : "-",
        "marker" : ".",
        "color"  : "C3",
      ...
      #  "color"  : "C6",
      #},
    ], ncol=2, loc="upper center", outside=True, columnspacing=1)
