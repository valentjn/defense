...
def main():
  for q in range(2):
    ...
    for t in range(d+1):
      ...
      if t == 0:   text = r"$\vfcnval = \vsurplus[(0)]$"
      elif t == 1: text = r"$\vsurplus^{(1)}$"
      else:        text = r"$\vsurplus^{(2)} = \vsurplus"
      ax.text(t * 1.3 + 0.5, -0.08, text, ha="center", va="top")
    
    helper.plot.plotArrow(ax, (0.74, -0.14), (1.66, -0.14))
    ax.text(1.2, -0.17, r"$\intpmatinv_1$", ha="center", va="top")
    helper.plot.plotArrow(ax, (1.94, -0.14), (2.86, -0.14))
    ax.text(2.4, -0.17, r"$\intpmatinv_2$", ha="center", va="top")
    helper.plot.plotArrowPolygon(
      ax, [0.5, 0.5, 3.1, 3.1], [-0.23, -0.42, -0.42, -0.23], "k-", cutOff=1)
    ax.text(1.8, -0.37, r"$\intpmatinv$", ha="center", va="bottom")
  ...
  fig = Figure.create(figsize=(2, 2), scale=1.8)
  ax = fig.gca()
  ...
  helper.plot.plotArrow(ax, (0.84, -0.16), (1.56, -0.16))
  ax.text(1.2, -0.19, r"$\intpmatinv_1$", ha="center", va="top")
  helper.plot.plotArrow(ax, (2.04, -0.16), (2.76, -0.16))
  ax.text(2.4, -0.19, r"$\intpmatinv_2$", ha="center", va="top")
  helper.plot.plotArrowPolygon(
    ax, [0.5, 0.5, 3.1, 3.1], [-0.3, -0.5, -0.5, -0.3], "k-", cutOff=1)
  ax.text(1.8, -0.44, r"$\intpmatinv$", ha="center", va="bottom")
