...
def plotJOrPolicy(solution, interpPolicy, discreteStateName, t, isSparseGrid,
  ...
  fig = Figure.create(figsize=(3, 3), scale=(0.50 if isSparseGrid else 0.50))
  ...
  if isSparseGrid:
    ...
    ax.plot(*X.T, "k.", clip_on=False,
            mec=backgroundColor, mew=0.4, ms=4)
  else:
  ...
  if isSparseGrid:
    ...
    ax.text(*trafo( 0.95,  0.95), r"\contour{{white}}{{{}}}".format(zLabel),
            ha="right",  va="top")
  else:
    ax.view_init(60, -120)
...
def main():
  ids = [14, 91, 96, 101]
  isSparseGrids = [False, True, True, True]
