...
def plotPolygon(ax, ts, ys, label, color, opacity):
  ...
  ax.add_patch(mpl.patches.Polygon(vertices, fc=lightColor, ec=color))
...
def main():
  ...
  for q, (id_, isStacked) in enumerate(zip(ids, isStackeds)):
    fig = Figure.create(figsize=(2.1, 2.25), preamble=r"""
\contourlength{0.7pt}
""")
    ax1 = fig.gca()
    ...
    ax1.set_yticks(yt)
    ax1.set_yticklabels([r"{:.0f}k".format(1e2*y) for y in yt])
    ...
    #ax1.text(*trafo( 0.95,  0.95), r"$d = {}$".format(d),
    #         ha="right",  va="top")
