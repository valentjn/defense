...
def generatePlot(q):
  ...
  figsize = ((3.1, 2.6) if q < 4 else (3, 2.5))
  ...
  if q == 4:
    ...
    plotOverlapMarkers(ax2, 10, 3e-1, [
      {"ls" : gridTypes[j][2], "color" : gridTypes[j][3]}
      #for j in [0, 1, 4, 5, 6, 7]
      for j in [0, 1]
    ])
    ax2.plot([10, 10], [1e-1, 8e-4], "k-", lw=0.8)
  ...
  ax.text(*trafo(0.5, 0.005), r"\contour{white}{Level of regular SG}",
          ha="center", va="bottom")
  ax.text(*trafo(0.02, 0.5),
          r"$\frac{\normLtwo{\objfun - \sgintp}}{\normLtwo{\objfun}}$",
          ha="left", va="center")
  ax.text(*trafo(1, 1), r"$f = \mathrm{Alp02}_2$", ha="right", va="top")
  
  #fig1.save(graphicsNumber=2*q-9)
  fig2.save()
...
def main():
  ...
  basisNames = [
    r"\rlap{Standard B-splines}",
    r"Not-a-knot B-splines",
    #r"$\bspl{\*l,\*i}{p}$",
    #r"$\bspl[\nak]{\*l,\*i}{p}$"
  ...
  helper.plot.addCustomLegend(ax, (
    [
      {
        "label"  : r"\rlap{Standard B-splines}",
        "ls"     : "-",
        "color"  : "C0",
      },
      None,
      {
        "label"  : r"Not-a-knot B-splines",
        "ls"     : "-",
        "color"  : "C1",
      },
    ] +
  ...
  ), ncol=3, loc="upper center", outside=True)
