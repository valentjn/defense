...
def plotSGWithVanishingPoints(X, L, I, l, i, p, ax, pos, size):
  d = X.shape[1]
...
def main():
  ...
  fig = Figure.create(figsize=(2, 2), scale=2.2)
  ax = fig.gca()
  ...
  for q in range(2):
    fig = Figure.create(figsize=(2, 2), scale=0.93)
    ax = fig.gca()
    
    if q == 0:
      basis1D = helper.basis.HierarchicalBSpline(p)
    else:
      basis1D = helper.basis.HierarchicalFundamentalSpline(p)
    ...
    #l = np.array([2, 1])
    #i = np.array([1, 1])
    #plotSGWithVanishingPoints(X, L, I, l, i, p, ax, (x, y), (1, 1))
