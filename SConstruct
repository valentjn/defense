...
for dir_ in dirs:
  ...
  if dir_ == "tex": env["BUILD_PDFS"]  = dirTargets[dir_]

# the PDFs depend on everything that is not in tex (these dependencies are
# handled by tex/SConscript) and out
env.Depends(env["BUILD_PDFS"], [dirTargets[dir_] for dir_ in dirs
                                if dir_ not in ["tex", "out"]])

# build flyer
env.SConscript(os.path.join("flyer", "SConscript"), exports=["env", "Helper"])

# build movie
env.SConscript(os.path.join("movie", "SConscript"), exports=["env", "Helper"])

# dummy targets for starting the presentation with `impressive`
command = r"""
impressive
...
--tracking
{}
out/defenseSlidesPart1.pdf
movie/out/movie_L.mkv
out/defenseSlidesPart2.pdf"""
command = command.strip().replace("\n", " ")
defenseSlideParts = ["out/defenseSlidesPart{}.pdf".format(q) for q in [1, 2]]
env.Command("impressive-debug", defenseSlideParts,
            command.format("--fake-fullscreen -g 1920x1080+3286+0"))
env.Command("impressive-fakefull-left", defenseSlideParts,
            command.format("--fake-fullscreen -g 1920x1080+0+0"))
env.Command("impressive-fakefull-right", defenseSlideParts,
            command.format("--fake-fullscreen -g 1920x1080+1920+0"))
env.Command("impressive-full", defenseSlideParts,
            command.format("--fullscreen"))
